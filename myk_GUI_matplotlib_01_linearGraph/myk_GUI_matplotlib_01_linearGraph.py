...
class BottomPanel(wx.Panel):
	def __init__(self, parent, top):
		...
		self.buttonMyk01 = wx.Button(self, -1, "test_Myk01", pos =(10,50))
		self.buttonMyk01.Bind(wx.EVT_BUTTON, self.SetButtonMyk01)
	...
	def SetButtonMyk01(self, event):
		min = self.textboxMinYAxis.GetValue() #テキストボックスから値取得
		max = self.textboxMaxYAxis.GetValue()
		self.x = np.arange(0,3,0.01)
		self.y = self.x * 2;
		self.graph.draw(self.x,self.y)
		cb = event.GetEventObject()
		print("[DEBUG]SetButtonMyk01: %s is clicked" % (cb.GetLabel()))
